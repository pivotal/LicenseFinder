...
module LicenseFinder
  class GitSubmodule < PackageManager
    ...
    def find_packages(path)
      ...
      return [] unless File.exists?(dotgitmodules_file_path)

      dotgitmodules = IniParse.parse( File.read(dotgitmodules_file_path))
      dotgitmodules.collect do |section|
        name = section.key.split('"')[1]
        [GitSubmodulePackage.new(name, @git_submodule_status[path.join(name).relative_path_from(project_path).to_s], path.join(section['path']), section['url'])] + find_packages(path.join(section['path']))
      end
    end
    ...
    def update_git_submodule_status
      ...
      stdout.split("\n").each do |gitmodule_line|
        commit, name, version = gitmodule_line.split(" ")
