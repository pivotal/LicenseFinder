...
As you record which packages have been approved, the executable task will write out a `dependency_decisions.yml` file (in the `doc/` directory by default -- see [Configuration](#configuration)).  This file records decisions you have made.

The `license_finder report` command will output human-readable reports that you could send to
your non-technical business partners, lawyers, etc.  You can choose the format of the report (csv, html or markdown); see `license_finder --help report` for details.  The output is sent to STDOUT, so you can save the reports wherever you want them.

The HTML report generated by `license_finder report --format html` shows a summary of the
project's dependencies and dependencies which need to be approved. The
project name at the top of the report can be set with
