...
Any decisions you make about approvals will be recorded in a
`dependency_decisions.yml` file (in the `doc/` directory by default — see
[Configuration](#configuration)).  This file must be committed to version
control.  Rarely, you will have to manually resolve conflicts in it.  In this
situation, keep in mind that each decision has an associated timestamp, and the
decisions are processed top-to-bottom, with later decisions overwriting or
appending to earlier decisions.
